...
componentDidMount() {
  axios.get(`/api/get_products/Accessories`).then( response => {
    console.log('get products response', response.data)
    this.props.addProducts(response.data);
    axios.get('/api/check_number_of_pages/Accessories').then( response => {
      let pagesArray = [];
...
getOtherProducts() {
  ...
  axios.get(`/api/get_other_products/Accessories/${offset}`).then( otherProductsResponse => {
...
export default withRouter(connectedApp(Accessories));