...
componentDidMount() {
  axios.get(`/api/get_products/Jeans`).then( response => {
    this.props.addProducts(response.data);
    axios.get('/api/check_number_of_pages/Jeans').then( response => {
      let pagesArray = [];
...
getOtherProducts() {
  ...
  axios.get(`/api/get_other_products/Jeans/${offset}`).then( otherProductsResponse => {
...
export default withRouter(connectedApp(Jeans));