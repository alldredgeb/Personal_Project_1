...
componentDidMount() {
  axios.get(`/api/get_products/Pants`).then( response => {
    console.log('get products response', response.data)
    this.props.addProducts(response.data)
    axios.get('/api/check_number_of_pages/Pants').then( response => {
      let pagesArray = [];
...
getOtherProducts() {
  ...
  axios.get(`/api/get_other_products/Pants/${offset}`).then( otherProductsResponse => {
...
export default withRouter(connectedApp(Pants));