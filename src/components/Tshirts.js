...
componentDidMount() {
  axios.get(`/api/get_products/T-Shirts`).then( response => {
    this.props.addProducts(response.data);
    axios.get('/api/check_number_of_pages/T-Shirts').then( response => {
      let pagesArray = [];
...
getOtherProducts() {
  ...
  axios.get(`/api/get_other_products/T-Shirts/${offset}`).then( otherProductsResponse => {
...
export default withRouter(connectedApp(Tshirts));