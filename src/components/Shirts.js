...
componentDidMount() {
  axios.get(`/api/get_products/Shirts`).then( response => {
    this.props.addProducts(response.data)
    axios.get('/api/check_number_of_pages/Shirts').then( response => {
      let pagesArray = [];
...
getOtherProducts() {
  ...
  axios.get(`/api/get_other_products/Shirts/${offset}`).then( otherProductsResponse => {
...
export default withRouter(connectedApp(Shirts));